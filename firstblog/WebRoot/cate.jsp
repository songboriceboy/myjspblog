...
//接受用户希望显示的页数（pageNow）
int scateid=Integer.parseInt(request.getParameter("id"));
...
ResultSet rs=bgdal.selectblogpagebycateid(scateid, pageSize, pageNow);
while(rs.next()){
...
<div class="blogs">
        ...
        String summary;
         ...
         %>
         <%=summary%>
          <p class="autor"><span class="lm f_l"><a href="cate.jsp?id=<%=rs.getInt("bCateid")%>"><%=rs.getString("bCatename")%></a></span><span class="dtime f_l"><%=rs.getString("bDate")%></span><span class="viewnum f_r">浏览（<a href="pageview.jsp?id=<%=rs.getInt("bId")%>"><%=rs.getInt("bClicks")%></a>）</span><span class="pingl f_r">评论（<a href="pageview.jsp?id=<%=rs.getInt("bId")%>"><%=rs.getInt("bComments")%></a>）</span></p>
        </ul>
...
%>
<a href = "cate.jsp?id=<%=scateid%>&pageNow=1" >首页</a>  &nbsp &nbsp &nbsp &nbsp &nbsp 
<a href = "cate.jsp?id=<%=scateid%>&pageNow=<%=pageNow-1%>" >上一页</a>  &nbsp &nbsp &nbsp 
<%}%>    
...
%>
<a href = "cate.jsp?id=<%=scateid%>&pageNow=<%=pageNow+1%>" >下一页</a>   &nbsp &nbsp &nbsp 
<a href = "cate.jsp?id=<%=scateid%>&pageNow=<%=pageCount%>" >尾页</a>   &nbsp &nbsp &nbsp 
<%}%>
