...
ResultSet pageRs=bgdal.selectblogpage(pageSize, pageNow);

while(pageRs.next()){

String bcatename="";
int bcateid=1;
if(pageRs.getString("bCatename")==null)
{
bcatename="没有分类";
}else{
bcatename=pageRs.getString("bCatename");
bcateid=pageRs.getInt("bCateid");
}
...
<div class="blogs">
        <figure><img src="<%=pageRs.getString("bPicsrc")%>"></figure>
        <ul>
          <h3><a href="pageview.jsp?id=<%=pageRs.getInt("bId")%>"><%=pageRs.getString("bTitle")%></a></h3>
         <%
         String summary;
         if(pageRs.getString("bContent").length()>10)       
         summary=pageRs.getString("bContent").substring(0,10);
         else
         summary=pageRs.getString("bContent");
         %>
         <%=summary%>
          <p class="autor"><span class="lm f_l"><a href="cate.jsp?id=<%=bcateid%>"><%=bcatename%></a></span><span class="dtime f_l"><%=pageRs.getString("bDate")%></span><span class="viewnum f_r">浏览（<a href="pageview.jsp?id=<%=pageRs.getInt("bId")%>"><%=pageRs.getInt("bClicks")%></a>）</span><span class="pingl f_r">评论（<a href="pageview.jsp?id=<%=pageRs.getInt("bId")%>"><%=pageRs.getInt("bComments")%></a>）</span></p>
        </ul>
...
%>
<a href = "index.jsp?pageNow=1" >首页</a>  &nbsp &nbsp &nbsp &nbsp &nbsp 
<a href = "index.jsp?pageNow=<%=pageNow-1%>" >上一页</a>  &nbsp &nbsp &nbsp 
<%}%>    
...
%>
<a href = "index.jsp?pageNow=<%=pageNow+1%>" >下一页</a>   &nbsp &nbsp &nbsp 
<a href = "index.jsp?pageNow=<%=pageCount%>" >尾页</a>   &nbsp &nbsp &nbsp 
<%}%>
...
第<%=pageNow%>页/共<%=pageCount%>页  
  ...
  <div class="r_box f_r">
   ...
   <font  size="4"  color="coral">  公告：<%=notice.getTitle() %>  </font>
